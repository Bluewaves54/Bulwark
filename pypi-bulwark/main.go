...
// Package main implements the PyPI Bulwark.
...
func run(ctx context.Context, cfgPath string, configExplicit, setupMode, uninstallMode, uninstallAllMode, updateMode, backgroundMode bool,
	...
	if backgroundMode {
		...
		fmt.Fprintf(out, "pypi-bulwark started in background (PID %d)\n", pid)
		return nil
	}
	...
	return runServer(ctx, srv, logger, "pypi-bulwark")
}

func main() {
	setupMode := flag.Bool("setup", false, "install Bulwark with best-practices config and configure pip")
	uninstallMode := flag.Bool("uninstall", false, "remove Bulwark and restore pip config")
